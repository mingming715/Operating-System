...
//	printf("syscall : %d\n", *(uint32_t *)(f->esp));
	...
	switch (syscall_no[0]) {	//System call Number
		...
		case SYS_CHDIR:
			check_user_vaddr(&syscall_no[1]);
			f->eax = chdir((char *)syscall_no[1]);
			break;
		case SYS_MKDIR:
			check_user_vaddr(&syscall_no[1]);
			f->eax = mkdir((char *)syscall_no[1]);
			break;
		case SYS_READDIR:
			check_user_vaddr(&syscall_no[1]);
			check_user_vaddr(&syscall_no[2]);
			f->eax = readdir((int)syscall_no[1], (char *)syscall_no[2]);
			break;
		case SYS_ISDIR:
			check_user_vaddr(&syscall_no[1]);
			f->eax = isdir((int)syscall_no[1]);
			break;
		case SYS_INUMBER:
			check_user_vaddr(&syscall_no[1]);
			f->eax = inumber((int)syscall_no[1]);
			break;
	}
}
int inumber(int fd)
{
	return inode_get_inumber(file_get_inode(thread_current()->fd[fd]));
}
int isdir(int fd)
{
